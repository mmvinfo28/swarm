...
const COMMANDS = {
  ...
  claim(root, a, flags) {
    ...
    // RULE: only complete a task assigned to you. Claim it first otherwise.
    if (t.assigned_to && t.assigned_to !== me.id) die(`not yours — that task belongs to someone else. Claim an open one first.`);
    if (!t.assigned_to) die(`not yours yet — run:  claim ${taskId}   then do the work, then done.`);
    ...
    out(`Done "${t.title}" — pending review by lead. Run \`next\` for more work.`, { ok: true, taskId });
  },
  ...
  outbox(root, a, flags) {
    ...
    const lines = rows.map(m => {
      const t = m.timestamp ? new Date(m.timestamp).toLocaleTimeString() : '';
      const who2 = nameOf(root, m.agent || m.from, cache);
      return `  ${t} [${m.box || 'out'}] ${who2}: ${m.content||''}`;
    });
