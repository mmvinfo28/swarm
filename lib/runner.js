...
// True when this worker actually shells out to `claude -p` (the only driver that
// depends on the CLI's on-disk oauth token).
function usesClaudeCli(provider) {
  return (process.env.SWARM_DRIVER || provider) === 'claude';
}

// Pause the worker on an auth failure and raise ONE actionable escalation telling
// the human to log in. error_reason='auth' lets the tick loop auto-resume the
// worker the moment the on-disk credentials become valid again.
function reportAuthFailure(root, agentId, agentName, log, detail) {
  const agent = reg.getAgent(root, agentId);
  reg.patch(root, agentId, { status: 'error', error_reason: 'auth' });
  if (agent && agent.auth_escalated) return; // already surfaced — don't spam
  reg.patch(root, agentId, { auth_escalated: true });
  const msg = `${agentName} cannot reach Claude: ${String(detail || 'auth failed').slice(0, 200)}`;
  try {
    const agentLoop = require('./agent-loop');
    const crypto = require('crypto');
    agentLoop.createEscalation(
      root, agentId,
      { id: crypto.randomUUID(), from: agentId, content: msg + ' — ' + authCheck.LOGIN_HINT },
      { reasons: [{ trigger: 'auth_failed' }], severity: 'critical' },
      authCheck.LOGIN_HINT
    );
  } catch (_) {}
  try { ioBus.deliverBroadcast(root, agentId, { type: 'status_update', content: `⚠ ${msg}. ${authCheck.LOGIN_HINT}` }); } catch (_) {}
  log('AUTH FAILED — worker paused (auto-resumes after `claude` /login). ' + String(detail || '').slice(0, 160));
}
...
const STRICT_SYSTEM = [
  ...
  '##SWARM:PLAN:what you intend##      propose a plan; wait for human approval before building',
  '##SWARM:SHUTDOWN:reason##          ask to stop; keep working until a human approves',
...
async function tick(ctx, opts) {
  ...
  // Graceful shutdown (Phase 3.2): a human/lead approved this worker stopping by
  // setting `stop`. Finish the tick we're in, mark offline, and signal the main loop
  // to exit — no kill mid-LLM-call.
  if (agent && agent.stop) {
    reg.updateStatus(root, agentId, 'offline', null);
    log('shutdown approved — exiting cleanly.');
    return { idle: true, shutdown: true };
  }
  ...
  if (agent && (agent.status === 'credits_exhausted' || agent.status === 'error')) {
    // Auth errors self-heal: the moment the on-disk credentials are valid again
    // (user ran /login), resume without a restart. A non-claude driver (e.g.
    // SWARM_DRIVER=fake) doesn't need CLI auth at all — clear the stale error.
    if (agent.error_reason === 'auth' && (!usesClaudeCli(ctx.provider) || authCheck.status().ok)) {
      reg.patch(root, agentId, { status: 'idle', error_reason: null, auth_escalated: false });
      log('auth restored — resuming.');
      try { ioBus.postRoom(root, agentId, `${agent.name} is back online (login restored).`, 'chat'); } catch (_) {}
    } else {
      if (opts.pull) log(`down (${agent.status}) — not calling the LLM. ` +
        (agent.error_reason === 'auth' ? 'Run `claude` + /login to auto-resume.' : 'Restart the worker to resume.'));
  ...
  try {
    ...
    // Auth failure (expired /login, bad key) → pause + one escalation; auto-resumes
    // once the user logs in again. Checked FIRST so a 401 never counts as "quota".
    if (authCheck.isAuthError(e.message)) {
      reportAuthFailure(root, agentId, agent.name, log, e.message);
      return { error: e.message, auth: true };
    }
...
async function main() {
  ...
  // Default LLM-call budget (usage cap) — applied once when unset. Configure via
  // config.yaml `default_max_calls`; override per-agent from the dashboard (budget
  // field). 0 = unlimited. Hitting the cap auto-pauses the worker.
  if (agent.max_calls === undefined || agent.max_calls === null) {
    let cap = 30;
    try {
      const yaml = require('./yaml');
      const cfg = yaml.parse(fs.readFileSync(path.join(root, '.swarm', 'config.yaml'), 'utf-8')) || {};
      if (cfg.default_max_calls !== undefined) cap = parseInt(cfg.default_max_calls, 10) || 0;
    } catch (_) {}
    reg.patch(root, agent.id, { max_calls: cap });
    if (cap) log(`LLM budget: ${cap} calls (change per-agent in the dashboard; 0 = unlimited)`);
  }

  // Auth preflight — if the claude CLI login is dead, surface it ONCE and poll
  // cheaply (no LLM calls) until the user logs in; then auto-resume.
  if (usesClaudeCli(a.provider)) {
    const auth = authCheck.status();
    if (!auth.ok) reportAuthFailure(root, agent.id, name, log, auth.reason);
    else if (agent.error_reason === 'auth') reg.patch(root, agent.id, { status: 'idle', error_reason: null, auth_escalated: false });
  }
  ...
  // Cadence source: SWARM_SYNC_SECONDS env (explicit per-process override) > config.yaml
  // sync_interval > 30s default.
  const SYNC_SECONDS = parseInt(process.env.SWARM_SYNC_SECONDS, 10) || tm.syncIntervalSeconds(root);
  ...
  do {
    ...
    let res;
    try { res = await tick(ctx, { pull: doPull }); } catch (e) { log('tick error: ' + e.message); }
    // Graceful shutdown approved during this tick — stop the loop cleanly.
    if (res && res.shutdown) { log('worker shut down.'); break; }
    if (a.once) break;
