...
function buildUserPrompt(root, agent, inbox, assigned) {
  const lines = [];
  lines.push('=== LIVE SWARM STATE ===');
  lines.push(`You are "${agent.name}" (id=${agent.id}). Capabilities: ${(agent.capabilities||[]).join(', ') || 'none'}.`);
  lines.push(hi.isLead(root, agent.id) ? 'You are the LEAD.' : 'You are a worker.');
  lines.push('');

  if (assigned.length) {
    lines.push('TASKS ASSIGNED TO YOU — do these now:');
    for (const t of assigned) {
      lines.push(`- id=${t.id} [${t.priority}] "${t.title}"${t.description ? ' — ' + t.description : ''}`);
    }
  ...
  if (inbox.length) {
    lines.push('YOUR INBOX (new messages):');
    for (const m of inbox.slice(0, 10)) {
      const tag = m.type === 'task_assignment' ? '[assignment] ' : '';
      lines.push(`- from ${String(m.from).slice(0,8)}: ${tag}${(m.content||'').slice(0, 300)}`);
    }
    lines.push('Reply to messages with ##SWARM:MSG:<from-id>:<reply>## if a response is needed.');
    lines.push('');
  }

  lines.push('Respond with a short note plus the ##SWARM:..## action markers for what you do now.');
  return lines.join('\n');
}
...
async function tick(ctx) {
  ...
  // Work-gate: nothing to do → cheap idle, NO LLM call.
  if (!inbox.length && !assigned.length) {
    reg.updateStatus(root, agentId, 'idle', null);
    log('idle — no inbox, no tasks; skipped LLM');
    return { idle: true };
  }
  ...
  const user = buildUserPrompt(root, agent, inbox, assigned);

  let resp;
  try {
    resp = await driver.run(system, user, { agentId, assignedTasks: assigned, inbox, root });
  } catch (e) {
  ...
  // Mark all inbox messages processed.
