...
function parseActions(text) {
  ...
  while ((m = MARKER_RE.exec(text)) !== null) {
    ...
    if (verb === 'CLAIM') {
      ...
      // Worker proposes a plan and blocks for human approval (Phase 3.3).
      if (payload) actions.push({ type: 'plan', content: payload });
    } else if (verb === 'SHUTDOWN') {
      // Worker asks to stop; a human/lead must approve before it exits (Phase 3.2).
      actions.push({ type: 'shutdown', reason: payload });
    } else if (verb === 'FILES') {
    ...
    try {
      if (a.type === 'claim') {
        ...
        if (t.assigned_to !== agentId) {
          // Allow a deferred git-mode claim: assigned_to isn't materialized yet but I'm
          // the claim-ticket winner. Otherwise it's genuinely not mine.
          if (!(!t.assigned_to && taskManager.getMyActiveClaim(root, a.taskId, agentId))) {
        ...
        // Phase 3.1: completing a task may unblock dependent pipeline stages — wake
        // them and redistribute so the next stage starts without waiting for a tick.
        try {
          const unblocked = taskManager.getNewlyUnblocked(root, a.taskId);
          if (unblocked.length) {
            ioBus.postRoom(root, agentId, `Unblocked: ${unblocked.map(u => `"${u.title}"`).join(', ')} (dependency done).`, 'chat');
            orchestrator.distributeNow(root);
          }
        } catch (_) {}
        ...
        // Plan-approval (Phase 3.3): raise an escalation tagged plan_approval so the
        // human can accept/redirect before the worker builds. Distinct trigger so the
        // dashboard can label it differently from a generic decision escalation.
        const agentLoop = require('./agent-loop');
        const crypto = require('crypto');
        const esc = agentLoop.createEscalation(
          root, agentId,
          { id: crypto.randomUUID(), from: agentId, content: a.content },
          { reasons: [{ trigger: 'plan_approval' }], severity: 'medium' },
          a.content
        );
        try { ioBus.postRoom(root, agentId, '[plan needs approval] ' + a.content, 'chat'); } catch (_) {}
        applied.push({ type: 'plan', ok: true, id: esc.id });

      } else if (a.type === 'shutdown') {
        // Graceful shutdown request (Phase 3.2): flag the agent and raise an actionable
        // escalation. The worker keeps running until a human/lead approves (sets stop),
        // then it exits cleanly on its next tick — no mid-LLM kill.
        agentRegistry.patch(root, agentId, { shutdown_requested: true });
        const agentLoop = require('./agent-loop');
        const crypto = require('crypto');
        const reason = a.reason || 'no reason given';
        const esc = agentLoop.createEscalation(
          root, agentId,
          { id: crypto.randomUUID(), from: agentId, content: `Requests shutdown: ${reason}` },
          { reasons: [{ trigger: 'shutdown_request' }], severity: 'low' },
          null
        );
        try { ioBus.postRoom(root, agentId, '[shutdown requested] ' + reason + ' — approve to stop this worker.', 'chat'); } catch (_) {}
        applied.push({ type: 'shutdown', ok: true, id: esc.id });
      }
    } catch (err) {
