...
function resolveBin() {
  ...
  try { if (spawnSync('codex', ['--version'], { stdio: 'ignore', timeout: 15000, windowsHide: true }).status === 0) { _bin = 'codex'; return _bin; } } catch (_) {}
  ...
  async run(systemPrompt, userPrompt) {
    ...
    if (bin) {
      ...
      const res = spawnSync(bin, args, {
        encoding: 'utf-8', shell: useShell, timeout: TIMEOUT, maxBuffer: 16 * 1024 * 1024, env: process.env, windowsHide: true,
      });
      let text = '';
