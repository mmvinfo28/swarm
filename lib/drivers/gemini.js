...
function cliAvailable() {
  try { return spawnSync('gemini', ['--version'], { stdio: 'ignore', shell: WIN, timeout: 15000 }).status === 0; }
...
module.exports = {
  ...
  async run(systemPrompt, userPrompt) {
    ...
    if (cliAvailable()) {
      const res = spawnSync('gemini', ['-p'], { input: prompt, encoding: 'utf-8', shell: WIN, timeout: TIMEOUT, maxBuffer: 16 * 1024 * 1024, env: process.env });
