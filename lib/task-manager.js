...
function reconcileClaims(swarmRoot, agentId) {
  const released = [];
  for (const t of listTasks(swarmRoot)) {
    // Only converge claim-acquired tasks (status 'in_progress'). Lead-`assign`ed tasks
    // are status 'assigned' and have their tickets pruned by assignTask, so we never
    // override a deliberate lead assignment with a stale claim ticket.
    if (t.status !== 'in_progress') continue;
    const winner = resolveClaimWinner(swarmRoot, t.id);
    if (!winner) continue;
    if (t.assigned_to === winner.agent_id) continue; // already agrees with the ticket
    const lostByMe = t.assigned_to === agentId && winner.agent_id !== agentId;
    // Rewrite the shared task file to the true winner (same on every machine).
    updateTask(swarmRoot, t.id, { assigned_to: winner.agent_id, status: 'in_progress' });
    if (lostByMe) released.push({ taskId: t.id, title: t.title, winner: winner.agent_id });
  }
  return released;
}
