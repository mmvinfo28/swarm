...
function findBestTask(swarmRoot, agentId) {
  ...
  const openTasks = listTasks(swarmRoot, { status: 'open' })
    ...
    .filter(t => !(multipleAgents && shouldAutoSplit(t)));
...
// Accepts a task object (preferred — checks parent_task) or (title, description).
// Split children are NEVER auto-split again: their title/description embed the
// parent title (commas and all), which made every distribute() tick re-split
// them — 250+ duplicate tasks per hour. parent_task set = terminal part.
function shouldAutoSplit(taskOrTitle, description) {
  let title = taskOrTitle;
  if (taskOrTitle && typeof taskOrTitle === 'object') {
    if (taskOrTitle.parent_task) return false;
    title = taskOrTitle.title;
    description = taskOrTitle.description;
  }
...
function autoSplitTask(swarmRoot, taskId, maxParts) {
  const task = getTask(swarmRoot, taskId);
  if (!task) return null;
  if (task.parent_task) return null; // belt-and-braces: never re-split a split child
