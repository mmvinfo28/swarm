...
function register(swarmRoot, name, provider, capabilities, owner) {
  // Defensive: some callers pass an options object as the 2nd arg. Extract fields so we
  // never store an object as `name` (which later breaks findByName's a.name.toLowerCase()).
  if (name !== null && typeof name === 'object') {
    console.warn('[agent-registry] register() called with object instead of positional args — extracting fields.');
    const o = name;
    name = o.name; provider = provider || o.provider; capabilities = capabilities || o.capabilities; owner = owner || o.owner;
  }
  if (typeof name !== 'string') name = name == null ? null : String(name);

  // Bug #7 fix: reuse existing agent with same name/role instead of creating duplicate.
  if (name) {
    const existing = findByName(swarmRoot, name);
    if (existing) {
      const updates = { last_seen: new Date().toISOString(), status: 'idle' };
      if (provider) updates.provider = provider;
      if (capabilities && capabilities.length) updates.capabilities = capabilities;
      return patch(swarmRoot, existing.id, updates);
    }
  }
...
// Patch arbitrary fields on an agent (capabilities, paused, max_calls, calls, role, …).
function patch(swarmRoot, agentId, fields) {
  const filePath = agentFile(swarmRoot, agentId);
  if (!fs.existsSync(filePath)) return null;
  const agent = yaml.parse(fs.readFileSync(filePath, 'utf-8'));
...
function findByName(swarmRoot, name) {
  ...
  return agents.find(a =>
    typeof a.name === 'string' && (a.name === name || a.name.toLowerCase() === name.toLowerCase())
  ) || null;
}
