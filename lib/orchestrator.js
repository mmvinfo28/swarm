...
function distribute(swarmRoot, leadId, opts) {
  ...
  // Auto-split: break big / multi-deliverable tasks into parts and hand one to
  // each worker, so a single agent (e.g. Codex) can't swallow the whole job.
  // Cap parts at the number of available workers — no point making more than
  // there are agents to take them.
  const maxParts = Math.max(2, agents.length);
  const toSplit = open.filter(t => taskManager.shouldAutoSplit(t.title, t.description));
  for (const t of toSplit) {
    const subs = taskManager.autoSplitTask(swarmRoot, t.id, maxParts);
    if (subs && subs.length) {
      open = open.filter(x => x.id !== t.id).concat(subs);
    }
  }
  ...
  for (const task of open) {
    ...
    // Deliver into the assignee's inbox (also mirrors to messages/ for the hook).
    ioBus.deliver(swarmRoot, best.id, {
      from: leadId,
      type: messageBus.MSG_TYPES.TASK_ASSIGNMENT,
      content: buildAssignmentPrompt(task),
      refs: { tasks: [task.id] },
    });
...
/**
 * Distribute right now via whoever the current lead is (electing one if needed).
 * Forced — used on task creation so a new task is split + cap-routed to the best
 * agent immediately, instead of waiting for the lead's next tick.
 */
function distributeNow(swarmRoot) {
  const leadId = ensureLead(swarmRoot);
  if (!leadId) return { ok: false, reason: 'no_lead', assignments: [] };
  return distribute(swarmRoot, leadId, { force: true });
}
