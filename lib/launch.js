  ...
  for (const name of names) {
    ...
    if (isAlive(pid)) {
      try {
        ...
        killTree(pid);
        console.log(`  stopped ${name} (pid ${pid}) + children`);
      }
...
function stopWorkers(root) {
  // Write the global stop flag so runners exit cleanly.
  try { const ioBusLib = require('./io-bus'); ioBusLib.setStopped(root); } catch (_) {}
  if (!fs.existsSync(runDir(root))) return [];
  const stopped = [];
  const names = fs.readdirSync(runDir(root)).filter(f => f.endsWith('.pid')).map(f => f.replace(/\.pid$/, ''));
  for (const name of names) {
...
module.exports = { startServer, startDashboard, startAgent, startWorker, status, stop, stopWorkers, isAlive, health };
