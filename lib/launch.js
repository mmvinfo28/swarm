...
function startWorker(root, provider, name, caps, interval) {
  ...
  const args = [path.join(PLUGIN_ROOT, 'lib', 'runner.js'),
    // Bug #6: 5s fast local poll for snappy message/task pickup. Git pull/push stays on
    // the slower SWARM_SYNC_SECONDS cadence, and idle ticks make zero LLM calls, so this
    // is cheap — it just stops inbox messages waiting up to 30s to be seen.
    '--root', root, '--provider', provider, '--name', name, '--caps', caps || '', '--interval', String(interval || 5)];
  ...
  if (provider === 'claude') {
    ...
    // Auth preflight — warn NOW instead of letting the worker discover it on tick 1.
    if ((process.env.SWARM_DRIVER || 'claude') === 'claude') {
      try {
        const auth = require('./auth-check').status();
        if (!auth.ok) console.log(`[worker] ⚠ ${auth.reason}\n[worker]   The worker starts paused-on-error and AUTO-RESUMES once you log in.`);
      } catch (_) {}
    }
  }
}

// First non-internal IPv4 — where a phone/laptop on the same wifi reaches this machine.
function lanIp() {
  try {
    const os = require('os');
...
function stop(root, opts) {
  opts = opts || {};
  // Write the global stop flag so runners exit cleanly.
  try { const ioBusLib = require('./io-bus'); ioBusLib.setStopped(root); } catch (_) {}

  // Kill any tracked processes (best-effort — may already be gone).
  const names = fs.existsSync(runDir(root))
    ? fs.readdirSync(runDir(root)).filter(f => f.endsWith('.pid')).map(f => f.replace(/\.pid$/, ''))
    : [];
  if (!names.length) console.log('  no tracked processes running');
  for (const name of names) {
    const pid = readPid(root, name);
    if (isAlive(pid)) {
      // Bug #5 fix: kill entire process tree (including child claude -p / LLM processes).
      try { killTree(pid); console.log(`  stopped ${name} (pid ${pid}) + children`); }
  ...
  // Phase 1.5: a plain stop should NOT throw away in-flight work. Reopen active tasks
  // (assigned/in_progress → open, claims pruned) so they redistribute and RESUME when
  // the swarm restarts. Completed tasks keep their result either way. Use --cancel only
  // when you really want to abandon the active work.
  if (opts.cancel) {
    const closed = closeAllActiveTasks(root);
    console.log(closed ? `  cancelled ${closed} active task(s)` : '  no active tasks to cancel');
  } else {
    const reopened = reopenActiveTasks(root);
    console.log(reopened ? `  reopened ${reopened} active task(s) — they resume on next start` : '  no active tasks to reopen');
  }
}

// Mark all non-done tasks (open / assigned / in_progress / split) as cancelled and
// drop their claim tickets. Returns the count closed.
function closeAllActiveTasks(root) {
  let tm; try { tm = require('./task-manager'); } catch (_) { return 0; }
  let count = 0;
  try {
    for (const t of tm.listTasks(root)) {
      if (t.status === 'done' || t.status === 'cancelled') continue;
      tm.updateTask(root, t.id, { status: 'cancelled', assigned_to: null });
      try { tm.pruneClaimsForTask(root, t.id); } catch (_) {}
      count++;
    }
  } catch (_) {}
  resetAgentsIdle(root);
  return count;
}

// Reopen tasks abandoned by stopped workers: assigned/in_progress → open, unassigned,
// claims pruned, so they're re-distributed when workers restart. Used by Stop workers
// (server stays up, so the work should resume — not be cancelled). Returns count.
function reopenActiveTasks(root) {
  let tm; try { tm = require('./task-manager'); } catch (_) { return 0; }
  let count = 0;
  try {
    for (const t of tm.listTasks(root)) {
      if (t.status !== 'assigned' && t.status !== 'in_progress') continue;
      tm.updateTask(root, t.id, { status: 'open', assigned_to: null });
      try { tm.pruneClaimsForTask(root, t.id); } catch (_) {}
      count++;
    }
  } catch (_) {}
...
// Reset every non-offline agent to idle with no current task.
function resetAgentsIdle(root) {
  try {
    const reg = require('./agent-registry');
    for (const a of reg.listAgents(root)) {
      if (a.status !== 'offline') { try { reg.updateStatus(root, a.id, 'idle', null); } catch (_) {} }
    }
  } catch (_) {}
}

// Stop only the LLM agents (worker-*/agent-*) — leaves server + dashboard running so
// you can still watch and restart. Useful when an LLM runs out of tokens.
function stopWorkers(root) {
  // Write the global stop flag so runners exit cleanly.
  try { const ioBusLib = require('./io-bus'); ioBusLib.setStopped(root); } catch (_) {}
  const stopped = [];
  const names = fs.existsSync(runDir(root))
    ...
    : [];
  for (const name of names) {
    if (!/^(worker|agent)-/.test(name)) continue;
    const pid = readPid(root, name);
    if (isAlive(pid)) { try { killTree(pid); stopped.push(name); } catch (_) {} }
    clearPid(root, name);
  }
  // Workers gone but server/dashboard stay up → reopen their tasks so a restart resumes them.
  reopenActiveTasks(root);
  return stopped;
}
...
function parseArgs(argv) {
  const out = { positional: [], wsPort: DEFAULT_WS_PORT, dashPort: DEFAULT_DASH_PORT, interval: 5, cancel: false };
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (argv[i] === '--cancel') out.cancel = true;
...
function main() {
  ...
  // Any START command clears the global stop flag — otherwise a prior stop() leaves
  // .swarm/.stopped set and freshly-spawned workers exit on their first tick (the flag
  // was never cleared anywhere). status/stop must NOT clear it.
  if (['stack', 'server', 'dashboard', 'agent', 'worker'].includes(String(cmd))) {
    try { require('./io-bus').clearStopped(root); } catch (_) {}
  }
    ...
    case 'stack':
      startServer(root, a.wsPort, () => {
        startDashboard(root, a.dashPort, () => {
          ...
          // Communication modes, auto-picked: same machine → WS + files (instant);
          // same wifi → open the dashboard from another device via the LAN URL below;
          // remote machines → git sync through the GitHub remote (works automatically
          // when `origin` exists). Nothing to configure.
          const ip = lanIp();
          if (process.env.SWARM_DASH_HOST === '0.0.0.0' && ip) {
            console.log(`LAN: dashboard reachable on your wifi at http://${ip}:${a.dashPort} (no auth — trusted network only)`);
          } else if (ip) {
            console.log(`LAN: to open the dashboard from a phone/laptop on the same wifi, restart with SWARM_DASH_HOST=0.0.0.0 (then http://${ip}:${a.dashPort})`);
          }
          try {
            const auth = require('./auth-check').status();
            if (!auth.ok) console.log(`⚠ AUTH: ${auth.reason}`);
          } catch (_) {}
...
module.exports = { startServer, startDashboard, startAgent, startWorker, status, stop, stopWorkers, closeAllActiveTasks, reopenActiveTasks, isAlive, health };
